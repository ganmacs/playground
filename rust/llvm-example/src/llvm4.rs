...
pub unsafe fn run() {
    ...
    // a + 1
    let name = CString::new("t_a").unwrap();
    let p_a = llvm::core::LLVMBuildLoad(builder, pointer_a, name.as_ptr());

    let int_type = llvm::core::LLVMInt64TypeInContext(context);
    let one = llvm::core::LLVMConstInt(int_type, 1, 0);

    let name = CString::new("add_value_then").unwrap();
    let tv = llvm::core::LLVMBuildAdd(builder, p_a, one, name.as_ptr());
    ...
    let int_type = llvm::core::LLVMInt64TypeInContext(context);
    let one = llvm::core::LLVMConstInt(int_type, 2, 0);

    let name = CString::new("add_value_els").unwrap();
    let ev = llvm::core::LLVMBuildAdd(builder, p_a, one, name.as_ptr());
